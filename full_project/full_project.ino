...
void enrollFingerprintProcess() {
  // Onesha START ENROLL kwenye line ya 1
  lcd.setCursor(0,0);
  lcd.print("START ENROLL      ");  // Extra spaces ku-clear mabaki ya maandishi ya zamani

  // Onesha FP ID unayoingiza kwenye line ya 2, usi-clear screen yote, tu clear line hii tu kwa spaces
  lcd.setCursor(0,1);
  lcd.print("FP ID: ");
  lcd.print(enteredFPID);
  // Clear leftover characters if any
  int leftover = 20 - (7 + enteredFPID.length()); // 7 = len("FP ID: ")
  ...
  if (key == '#') {
    ...
    if (isFingerprintIDUsed(fpID) || hasFPModel(fpID)) {
      lcd.setCursor(0,1);
      lcd.print("FP ID Tayari      "); // Clear line 2 with spaces
      delay(2000);
      currentState = SHOW_MENU;
      enteredFPID = "";
      return;
    ...
    enteredFPID = "";
    lcd.setCursor(0,1);
    lcd.print("                    "); // Clear line 2 with spaces
  } else {
